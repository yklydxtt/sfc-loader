        ...
        if (parentKey && parentKey.name === this.childKey) {
            ...
            if (key.name === 'type') {
                if (t.isIdentifier(node)) {
                    ...
                    if (!elements.length) {
                        throw new Error(`Providing a type for the ${this.childKey} prop is a good practice.`);
                    }
                    /** 
                    ...
                    */
                    ...
                    throw new Error(`The type in ${this.childKey} prop only supports identifier or array expression, eg: Boolean, [String]`);
                }
            }
            ...
            if (t.isLiteral(node)) {
                ...
                console.log(this.state.props[this.childKey].value);
                collectObj(path,this.state.props[this.childKey].value)
            }
        }
    },
...
function collectObj(path,state){
    path.traverse({
        ObjectProperty(childPath){
            ...
            if(t.isLiteral(childVal)){
                state[childKey]=childVal.value;
            }else if(t.isObjectExpression(childVal)){
                state[childKey]={};
                collectObj(childPath,state[childKey]);
            }
        }
    })
}
                ...
                if (parentNode.key && parentNode.key.name === parentKey) {
                    ...
                    if (!state.props[childKey]) {
                        if (t.isArrayExpression(childVal)) {
                            ...
                            throw new Error(`Not supports expression for the ${this.childKey} prop in props.`);
                        }
                    }
                }
            }
